...
**功能**：
- AI 维护当前阶段（项目启动 / 迭代X-规划 / 迭代X-设计 / 迭代X-实现 / 迭代X-验证）
- 人类输入与阶段不符时，AI 主动引导
- 提供切换选项：立即处理 or 记录灵光
...
| 类型 | 判断标准 | 示例 |
|------|----------|------|
| 符合阶段 | 请求属于当前阶段的正常产出 | 规划阶段讨论功能范围 |
| 不符合阶段 | 请求属于其他阶段的产出 | 规划阶段要求写代码 |
...
| 时间 | 类型 | 阶段 | 内容 | AI归纳 |
|------|------|------|------|--------|
| 2024-12-17 10:30 | 需求 | 迭代1-规划 | 原始输入内容 | 一句话总结 |
```
...
## 已处理
- [x] 已处理的灵光 (记录于: 2024-12-16, 处理于: 2024-12-17)
```
...
| 时间 | 变更类型 | 涉及文件 | 变更内容 |
|------|----------|----------|----------|
| 2024-12-17 14:00 | 实现倒逼 | auth模块 | 接口增加 token 参数 |
...
**AI 操作规范**：
- 记录输入日志：直接读取 input-log.md，在表格末尾追加新行
- 记录灵光：直接读取 spark-box.md，在"待处理"下追加新条目
- 记录 pending docs：直接读取 pending-docs.md，在"待同步"表格追加
...
| 输入日志 | 检查 `.flow/input-log.md` | 包含本迭代的关键决策记录，格式正确 |
| 灵光收集 | 检查 `.flow/spark-box.md` | 灵光有记录，阶段结束时有处理 |
| 流程引导 | 人工测试：在规划阶段要求写代码 | AI 识别并引导，不直接执行 |
| 状态可读 | IDE 打开 `.flow/state.json` | 节点可折叠，结构清晰 |
| 跨Session | 新会话让 AI 读取 state.json | AI 能理解当前状态并继续工作 |
| Pending Docs | Commit 前检查 | AI 主动提醒处理 pending docs |
| 文件直接编辑 | AI 记录输入/灵光 | 不调用脚本，直接编辑 Markdown 文件 |
| npm scripts | 运行 status 和 validate | 输出符合预期 |
...
*创建时间：2024-12-17*
*基于：产品PRD v0.5*
*更新内容：v1.1 重构 npm scripts 设计，新增文件操作格式规范；v1.2 新增流程文档裁剪说明（CLAUDE.md 暂代）*

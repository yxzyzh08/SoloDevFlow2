...
**Pending Docs 机制（心流保护）**：

编码过程中发现设计问题，停下来改文档会打断心流。允许以下流程：

```
发现设计问题 → 记录到 .flow/pending-docs.md → 继续编码
                    ↓
              Git Commit 前
                    ↓
         AI 根据代码 Diff 反向更新文档 → 清空 pending-docs.md
```

**规则**：
- Pending Docs 只是临时缓冲，不是逃避文档更新的借口
- 必须在当次 Commit 前清空，不允许跨 Commit 累积
- AI 在 Commit 前主动检查并处理 pending-docs.md
...
**状态管理说明**：
- `state.json` 是唯一数据源，包含完整状态
- `status.md` 是 `state.json` 的人类可读视图，由 AI 或脚本生成
- 避免双向同步问题，确保数据一致性

**跨 Session 状态连续性**：
- Claude Code 每次启动是全新上下文
- AI 必须在每次对话开始时主动读取 `.flow/status.md` 恢复状态
...
- [ ] 状态可读、可追溯（人类能通过 status.md 了解当前状态）
...
*创建时间：2024-12-16*
*更新时间：2024-12-17*
*更新内容：v0.2 增加重要认知、AI判断规则、状态管理单一数据源；v0.3 增加人机职责分工、上下文加载策略、Pending Docs机制、影响分析局限性说明、YAGNI原则、模块文档可选规则*

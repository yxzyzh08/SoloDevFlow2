...
### 3.2 设计阶段流程（code 类型 Feature）

```
Feature Spec 完成 → 评估 Design Depth → L0? ─是→ 直接进入实现
                                           │
                                           否
                                           ↓
                                  L1/L2/L3 → 调用 /write-design → 输出设计文档
```
...
| 级别 | 条件 | 产出 |
|------|------|------|
| L0 | 极简单，半天内完成，无需架构决策 | 无设计文档 |
| L1 | 简单，1-2 天，边界清晰 | 轻量设计 |
| L2 | 中等，3-7 天，涉及多模块 | 标准设计 |
| L3 | 复杂，高风险，核心架构 | 详细设计 |
